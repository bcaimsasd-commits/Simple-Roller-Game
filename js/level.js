  ...
  fetch("data/pieces.json")
    ...
    .catch(function (error) {
      document.getElementById("message").textContent =
        "Could not load the level files.";
      console.error(error);
    });
};
...
Level.build = function (levelNumber) {
  ...
  for (var pieceIndex = 0; pieceIndex < level.pieces.length; pieceIndex++) {
    var piece = Level.pieces[level.pieces[pieceIndex]] || Level.pieces.flat;
    for (var pieceRow = 0; pieceRow < CONFIG.ROWS; pieceRow++) {
      Level.grid[pieceRow] += piece[pieceRow];
    }
  }
...
Level.charAt = function (col, row) {
  if (row < 0 || row >= CONFIG.ROWS || col < 0 || col >= Level.cols) {
    return ".";
  }
